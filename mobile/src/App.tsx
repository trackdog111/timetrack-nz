...
function AppContent({ authHook }: { authHook: ReturnType<typeof useAuth> }) {
  ...
  const submitExpense = isDemoMode 
    ? async (_amount: number, _category: any, _date: Date, _photoBase64?: string, _note?: string, _worksiteId?: string, _worksiteName?: string) => { showDemoToast('Submitting expenses'); return false; } 
    : expensesHook.submitExpense;
  const updateExpense = isDemoMode
    ? async (_expenseId: string, _amount: number, _category: any, _date: Date, _note?: string, _worksiteId?: string, _worksiteName?: string) => { showDemoToast('Updating expenses'); return false; }
