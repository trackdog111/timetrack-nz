...
interface Shift { id: string; userId: string; userEmail: string; clockIn: Timestamp; clockOut?: Timestamp; clockInLocation?: Location; clockOutLocation?: Location; clockInPhotoUrl?: string; locationHistory: Location[]; breaks: Break[]; travelSegments?: TravelSegment[]; jobLog: JobLog; status: 'active' | 'completed'; manualEntry?: boolean; editedAt?: Timestamp; editedBy?: string; editedByEmail?: string; finalized?: boolean; finalizedAt?: Timestamp; finalizedBy?: string; finalizedByEmail?: string; }
interface EmployeeSettings { gpsTracking: boolean; gpsInterval: number; requireNotes: boolean; chatEnabled: boolean; autoTravel?: boolean; autoTravelInterval?: number; detectionDistance?: number; }
...
interface CompanySettings { field1Label: string; field2Label: string; field3Label: string; managerDisplayName: string; paidRestMinutes: number; payWeekEndDay: number; photoVerification: boolean; }

const defaultSettings: EmployeeSettings = { gpsTracking: true, gpsInterval: 10, requireNotes: false, chatEnabled: true, autoTravel: false, autoTravelInterval: 2, detectionDistance: 200 };
const defaultCompanySettings: CompanySettings = { field1Label: 'Notes', field2Label: 'Materials', field3Label: 'Other', managerDisplayName: 'Manager', paidRestMinutes: 10, payWeekEndDay: 0, photoVerification: false };
    ...
    <main style={{ minHeight: '100vh', background: theme.bg }}>
      ...
      <div style={{ marginLeft: isMobile ? 0 : '260px', padding: isMobile ? '72px 16px 16px' : '24px 32px' }}>
        ...
        {view === 'mysheet' && <div>
          ...
          {/* Auto-Travel Settings for Manager */}
          {(() => {
              ...
              <div style={{ ...styles.card, marginTop: '16px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fit, minmax(180px, 1fr))', gap: '12px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: myEmployee.settings?.autoTravel ? theme.travelBg : theme.cardAlt, padding: '12px', borderRadius: '8px' }}>
                    ...
                    <button onClick={() => updateSettings(myEmployee.id, { autoTravel: !myEmployee.settings?.autoTravel })} style={{ width: '50px', height: '26px', borderRadius: '13px', border: 'none', cursor: 'pointer', background: myEmployee.settings?.autoTravel ? theme.travel : '#cbd5e1', position: 'relative' }}><span style={{ position: 'absolute', top: '3px', width: '20px', height: '20px', borderRadius: '50%', background: 'white', left: myEmployee.settings?.autoTravel ? '27px' : '3px', transition: 'left 0.2s' }} /></button>
                  </div>
