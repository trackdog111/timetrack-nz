...
export default function App() {
  ...
  const myClockOut = async () => { if (!myShift) return; const location = await getMyCurrentLocation(); let ub = [...(myShift.breaks || [])]; const ai = ub.findIndex(b => !b.endTime && !b.manualEntry); if (ai !== -1 && breakStart) { ub[ai] = { ...ub[ai], endTime: Timestamp.now(), durationMinutes: Math.round((Date.now() - breakStart.getTime()) / 60000) }; } let ut = [...(myShift.travelSegments || [])]; const ti = ut.findIndex(t => !t.endTime); if (ti !== -1 && myTravelStart) { ut[ti] = { ...ut[ti], endTime: Timestamp.now(), endLocation: location || undefined, durationMinutes: Math.round((Date.now() - myTravelStart.getTime()) / 60000) }; } await updateDoc(doc(db, 'shifts', myShift.id), { clockOut: Timestamp.now(), clockOutLocation: location, breaks: ub, travelSegments: ut, 'jobLog.field1': myField1, 'jobLog.field2': myField2, 'jobLog.field3': myField3, status: 'completed' }); setSuccess('Clocked out!'); };
