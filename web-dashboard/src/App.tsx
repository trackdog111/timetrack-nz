...
export default function App() {
  ...
  useEffect(() => { if (!user) return; const myActive = activeShifts.find(s => s.userId === user.uid); setMyShift(myActive || null); if (myActive) { const ab = myActive.breaks?.find(b => !b.endTime && !b.manualEntry); setOnBreak(!!ab); setBreakStart(ab ? ab.startTime.toDate() : null); setMyField1(myActive.jobLog?.field1 || ''); } else { setOnBreak(false); setBreakStart(null); } }, [user, activeShifts]);
  ...
  const myClockIn = async () => { if (!user) return; const location = await getMyCurrentLocation(); await addDoc(collection(db, 'shifts'), { userId: user.uid, userEmail: user.email, clockIn: Timestamp.now(), clockInLocation: location, locationHistory: location ? [location] : [], breaks: [], travelSegments: [], jobLog: { field1: '', field2: '', field3: '' }, status: 'active' }); setSuccess('Clocked in!'); };
  const myClockOut = async () => { if (!myShift) return; const location = await getMyCurrentLocation(); let ub = [...(myShift.breaks || [])]; const ai = ub.findIndex(b => !b.endTime && !b.manualEntry); if (ai !== -1 && breakStart) { ub[ai] = { ...ub[ai], endTime: Timestamp.now(), durationMinutes: Math.round((Date.now() - breakStart.getTime()) / 60000) }; } let ut = [...(myShift.travelSegments || [])]; const ti = ut.findIndex(t => !t.endTime); if (ti !== -1 && myTravelStart) { ut[ti] = { ...ut[ti], endTime: Timestamp.now(), endLocation: location || undefined, durationMinutes: Math.round((Date.now() - myTravelStart.getTime()) / 60000) }; } await updateDoc(doc(db, 'shifts', myShift.id), { clockOut: Timestamp.now(), clockOutLocation: location, breaks: ub, travelSegments: ut, 'jobLog.field1': myField1, status: 'completed' }); setSuccess('Clocked out!'); };
